...
def parse_common_or_data(lines=None, datadic=None, inverse=False, ofs=0,
                         what='common'):
    if not inverse:
        datadic = {}
...
def parse_subentry(lines=None, datadic=None, inverse=False, ofs=0):
    if not inverse:
        datadic = {}
        ...
        while ofs < len(lines) and read_str_field(lines[ofs], 0) != 'ENDSUBENT':
            ...
            if curfield == 'COMMON':
                commonsec, ofs = parse_common_or_data(lines, datadic, inverse, ofs,
                                                      what='common')
                datadic['COMMON'] = commonsec

            if curfield == 'DATA':
                datasec, ofs = parse_common_or_data(lines, datadic, inverse, ofs,
                                                    what='data')
        ...
        if 'COMMON' in datadic:
            ...
            curlines, ofs = parse_common_or_data(lines, curdic, inverse, ofs, 
                                                 what='common')
            lines.extend(curlines)
        else:
...
def parse_entry(lines=None, datadic=None, inverse=False, ofs=0):
    if not inverse:
        datadic = {'subentries': []}
        ...
        while ofs < len(lines) and read_str_field(lines[ofs], 0) != 'ENDENTRY':
            if read_str_field(lines[ofs], 0) == 'SUBENT':
                subent, ofs = parse_subentry(lines, None, inverse, ofs)
        ...
        lines = []
        lines.extend(write_str_field('', 0, 'ENTRY'))
        ofs += 1
...
def parse_file(lines=None, datadic=None, inverse=False, ofs=0):
    if not inverse:
        datadic = {'entries': []}
        while ofs < len(lines):
            if read_str_field(lines[ofs], 0) == 'ENTRY':
